...
async function postObservation(pid, entry, type) {
  // Default options are marked with *
  var pid = document.getElementById('patient_id').innerHTML;
  var formData = new FormData();
  formData.append("pid", pid);
...
async function downloadFhir(pid) {
  // Default options are marked with *
  var pid = document.getElementById('patient_id').innerHTML;
  var formData = new FormData();
  formData.append("pid", pid);

  fetch('https://jobrien35-cs6440.herokuapp.com/api/v1/nutrition/observation?pid='.concat(pid), {mode: 'cors'})
    .then(
      function(response) {
        console.log(response.status)
        // Examine the text in the response
