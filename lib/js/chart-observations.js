...
document.getElementById('patient_id').innerHTML = retrievePid();
...
function parsePatientObservations(pid) {
  var count = 0;
  var d = null;
  var qstr = 'https://r4.smarthealthit.org/Observation?patient='.concat(pid)
  var c = fetch(qstr, {mode: 'cors'})
  .then(data=>{
    return data.json()
  })
    .then(res=>{
  ...
  fetch("https://jobrien35-cs6440.herokuapp.com/api/v1/nutrition/mfp", {
    ...
    .then(
      function(response) {
        ...
        response.json().then(function(data) {
          console.log(data);
          document.getElementById("patient_id").innerHTML = data.message
        });
      }
    )
    .catch(function(err) {
