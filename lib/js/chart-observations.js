...
document.getElementById('download_fhir').addEventListener('click', downloadFhir);
document.getElementById('reset_pid').addEventListener('click', resetPid);
...
function resetPid() {
  var resetp = '...';
  document.getElementById('patient_id').innerHTML = resetp;
  storePid(resetp)
  console.log('reset pid: ' + retrievePid());
}


function extendData(div_name, value, date) {
  console.log(div_name + ' ' + value + ' ' + date)
...
function extendBPData(div_name, systolic_value, diastolic_value, date) {
  console.log('bp sys ' + systolic_value + 'dias ' + diastolic_value + ' ' + date)
  Plotly.extendTraces(div_name, { // div name
...
function addSodiumEntry() {
  var value = parseInt(document.getElementById('sodium_input').value); // pass mg to backend since mfp uses it too
  var val = value/1000; // match loinc units of grams
  if (Number.isNaN(value)) {
    console.log('sodium invalid');
    return
  }
  var entry = {
  ...
  extendData('sodium_div', val, entry.date);
  console.log('new sodium point:');
  console.log(entry);

  postObservation(entry, 'sod');
...
function addPotassiumEntry() {
  ...
  var value = parseInt(document.getElementById('potassium_input').value); // pass mg to backend since mfp uses it too
  var val = value/1000; // match loinc units of grams
  if (Number.isNaN(value)) {
  ...
  extendData('potassium_div', val, entry.date);
  console.log('new potassium point:');
  console.log(entry);

  postObservation(entry, 'pot')
}
...
function addBPEntry() {
  ...
  if (Number.isNaN(systolic_value)) {
    console.log('systolic invalid');
    return
  }
  if (Number.isNaN(diastolic_value)) {
    console.log('diastolic invalid');
    return
  }
  var entry = {
    "date" : new Date().toISOString(),  // When the entry was made (YYYY-MM-DDTHH:mm:ss.SSSSZ)
    ...
    .then(res=>{
      ...
      d.entry.map((idx)=> {
        ...
        FHIR.oauth2.ready().then(function(client) {
          ...
          console.log('effective date ' + curr_item.effectiveDateTime)
          ...
          if (curr_item.resourceType == 'Observation' && curr_item.code.text == "Sodium intake 24 hour Estimated") {
            ...
            extendData('sodium_div', getQuantityValueOnly(curr_item), obs_date.toISOString())
          }
          ...
          if (curr_item.resourceType == 'Observation' && curr_item.code.text == "Potassium intake 24 hour Estimated") {
            ...
            extendData('potassium_div', getQuantityValueOnly(curr_item), obs_date.toISOString())
          }
          count = count + 1;
    ...
    .then(
      function(response) {
        ...
        response.json().then(function(data) {
          ...
          parsePatientObservations(data.pid);
        });
      }
    )
    .catch(function(err) {
...
async function postObservation(entry, type) {
  // Default options are marked with *
  var pid = document.getElementById('patient_id').innerHTML;
  var formData = new FormData();
  formData.append("pid", pid);
  ...
  console.log(type == 'sod')
  console.log(type === 'sod')
  if (type == 'bp') {
    formData.append('val', entry.systolic_value);
    formData.append('val2', entry.diastolic_value);
  }
  if (type == 'sod' || type == 'pot') {
    formData.append('val', entry.value);
  }
  console.log(formData)
  fetch('https://jobrien35-cs6440.herokuapp.com/api/v1/nutrition/observation', {
    method: 'POST', mode: 'cors', body: formData})
    .then(
      function(response) {
        console.log(response.status)
...
async function downloadFhir() {
  var pid = document.getElementById('patient_id').innerHTML;
  var url = 'https://jobrien35-cs6440.herokuapp.com/api/v1/nutrition/download?pid='.concat(pid)
  var filename = pid
  filename = pid.concat('.json')
  fetch(url).then(function(t) {
    return t.blob().then((b) => {
      var a = document.getElementById("download_fhir");
